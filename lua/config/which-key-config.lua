-- ~/.config/nvim/lua/config/which-key-config.lua
-- Config for which-key with dynamic layout-based filtering

local M = {}

-- Filter function that shows only relevant keys for current layout
local function layout_filter(mapping)
    -- If LangmapHelper not loaded yet, show everything
    if not _G.LangmapHelper or not _G.LangmapHelper.current_layout then
        return true
    end

    local layout = _G.LangmapHelper.current_layout
    local lhs = mapping.lhs or ""

    -- Always show leader key itself and function keys
    if lhs == "<leader>" or lhs == " " or lhs:match("^<F%d+>") or lhs:match("^<A%-") then
        return true
    end

    -- Check if mapping contains Ukrainian letters
    local has_ua = false
    for ua_char, _ in pairs(_G.LangmapHelper.ua_to_en or {}) do
        if lhs:find(ua_char, 1, true) then
            has_ua = true
            break
        end
    end

    -- Check if mapping contains English letters
    local has_en = lhs:find("[a-zA-Z]")

    -- Filter based on current layout
    if layout == "ua" then
        -- In Ukrainian layout: show only Ukrainian keys and neutral keys
        return has_ua or (not has_ua and not has_en)
    elseif layout == "en" then
        -- In English layout: show only English keys and neutral keys
        return has_en or (not has_ua and not has_en)
    end

    -- Default: show everything if layout unknown
    return true
end

-- Dynamic title based on layout
local function get_title()
    if _G.LangmapHelper and _G.LangmapHelper.current_layout then
        if _G.LangmapHelper.current_layout == "ua" then
            return " 🇺🇦 Which Key (Українська) "
        else
            return " 🇬🇧 Which Key (English) "
        end
    end
    return " 🗺️  Which Key "
end
...
M.opts = {
    ...
    win = {
        no_overlap = true,
        padding = {3, 3},
        -- Use function for dynamic title
        title = get_title,
        title_pos = "center",
        zindex = 1000,
        wo = {winblend = 10}
